...
/** Token savings recorded by this CLI: raw GraphQL JSON minus rendered output. */
export async function gainCommand(): Promise<string> {
  const entries = readGainLog();
  if (entries.length === 0) {
    return renderOutput([
      encode({ gain: "no invocation recorded", log: gainLogPath() }),
      renderHelp([
        "Run a read command (`monday-axi ticket list`) to feed the log",
...
/** A reduce, not `Math.min(...)`: the log grows unbounded and spreading it would overflow the stack. */
