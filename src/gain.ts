...
const CLI = "monday-axi";

/**
 * One invocation of the CLI. `raw` is what an agent would have ingested by
 * calling the Monday GraphQL API itself, `out` what it ingests through this
 * CLI. Only integers and the sub-command name are ever recorded — never
 * arguments, flag values, board or item ids, URLs or payload fragments.
 */
